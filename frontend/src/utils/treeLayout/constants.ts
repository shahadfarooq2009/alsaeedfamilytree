...
/** Width from current scale tier — fixed per tier, not label-driven. */
export function cardWidthForMember(member: {
  ...
  return isFounderMember(member) ? scale.founderCardWidth : scale.cardWidth;
}

export function cardHeightForMember(member: { fatherId: number | null; generation: number }): number {
  const scale = getLayoutScale();
  return isFounderMember(member) ? scale.founderCardHeight : scale.cardHeight;
}

export function cardFootprintForMember(member: { fatherId: number | null; generation: number }): number {
  const scale = getLayoutScale();
  if (isFounderMember(member)) {
    return scale.founderCardHeight + scale.founderBadgeOverhang;
  }
  return scale.cardHeight + scale.badgeOverhang;
}
